...
namespace ns3 {
  ...
  NS_LOG_COMPONENT_DEFINE("DENBasicServiceV1");

  DENBasicServiceV1::DENBasicServiceV1()
  {
  ...
  bool
  DENBasicServiceV1::CheckMainAttributes()
  {
    return m_station_id!=ULONG_MAX && m_stationtype!=LONG_MAX;
  }

    template<typename MEM_PTR> void
     DENBasicServiceV1::setDENTimer(Timer &timer,Time delay,MEM_PTR callback_fcn,DEN_ActionID_t actionID) {
      if(timer.IsRunning ())
        {
           timer.Cancel();
        }
  ...
  DENBasicServiceV1::fillDENM(asn1cpp::Seq<DENMV1> &denm, denData &data, const DEN_ActionID_t actionID,long referenceTimeLong)
  {
    ...
    asn1cpp::setField(denm->header.protocolVersion,1);
    ...
    if(data.isDenmSituationDataSet ())
      {
         ...
         auto situation_seq = asn1cpp::makeSeq(SituationContainerV1);
         ...
         if(situation_data.getData ().linkedCauseCode.isAvailable ())
           {
             auto causeCode = asn1cpp::makeSeq(CauseCodeV1);
             ...
             for(size_t i=0;i<situation_data.getData ().eventHistory.getData ().size ();i++)
               {
                 ...
                 auto eventPoint_seq = asn1cpp::makeSeq(EventPointV1);
    ...
    if(data.isDenmLocationDataSet ())
      {
        ...
        auto location_seq = asn1cpp::makeSeq(LocationContainerV1);

        auto traces = asn1cpp::makeSeq(TracesV1);
        for(size_t i=0;i<location_data.getData ().traces.size ();i++)
          {
            auto pathHistory = asn1cpp::makeSeq(PathHistoryV1);
            for(size_t j=0;j<location_data.getData ().traces[i].size ();j++)
              {
                auto pathPoint = asn1cpp::makeSeq(PathPointV1);
        ...
        if(location_data.getData ().eventSpeed.isAvailable ())
          {
            auto eventSpeed = asn1cpp::makeSeq(SpeedV1);
        ...
        if(location_data.getData ().eventPositionHeading.isAvailable ())
          {
            auto eventHeading = asn1cpp::makeSeq(HeadingV1);
    ...
    if(data.isDenmAlacarteDataSet ())
      {
        ...
        auto alacarte_seq = asn1cpp::makeSeq(AlacarteContainerV1);
        ...
        if(alacarte_data.getData ().impactReduction.isAvailable ())
          {
            auto impactReduction = asn1cpp::makeSeq(ImpactReductionContainerV1);
        ...
        if(alacarte_data.getData ().roadWorks.isAvailable ())
          {
            auto roadworks = asn1cpp::makeSeq(RoadWorksContainerExtendedV1);
            ...
            if(roadworks_data.innerhardShoulderStatus.isAvailable () ||
              ...
              {
                auto closedLanes = asn1cpp::makeSeq (ClosedLanesV1);
            ...
            if(roadworks_data.causeCode.isAvailable ())
              {
                auto incidentInd_seq = asn1cpp::makeSeq(CauseCodeV1);
            ...
            if(roadworks_data.recommendedPath.isAvailable ())
              {
                for(size_t i=0;i<roadworks_data.recommendedPath.getData ().size ();i++)
                  {
                    auto refPos_seq = asn1cpp::makeSeq(ReferencePositionV1);
            ...
            if(roadworks_data.startingPointSpeedLimit.isAvailable ())
              {
                auto startPointSLimit_seq = asn1cpp::makeSeq(DeltaReferencePositionV1);
            ...
            if(roadworks_data.referenceDenms.isAvailable ())
              {
                for(size_t i=0;i<roadworks_data.referenceDenms.getData ().size ();i++)
                  {
                    auto actionId_seq = asn1cpp::makeSeq(ActionIDV1);
        ...
        if(alacarte_data.getData ().stationaryVehicle.isAvailable ())
          {
            auto stationary_veh_seq = asn1cpp::makeSeq(StationaryVehicleContainerV1);
            ...
            if(stationary_veh_data.causeCode.isAvailable ())
              {
                auto stationaryCause_seq = asn1cpp::makeSeq(CauseCodeV1);
            ...
            if(stationary_veh_data.carryingDangerousGoods.isAvailable ())
              {
                ...
                auto dangerous_seq = asn1cpp::makeSeq(DangerousGoodsExtendedV1);
            ...
            if(stationary_veh_data.vehicleIdentification.isAvailable ())
              {
                auto vehicleId_seq = asn1cpp::makeSeq(VehicleIdentificationV1);
    ...
    return DENMV1_NO_ERROR;
  }

  template <typename T> int
  DENBasicServiceV1::asn_maybe_assign_optional_data(T *data, T **asn_structure, std::queue<void *> &ptr_queue)
  {
    if(data==NULL) {
        return 0;
      }
  ...
  DENBasicServiceV1::DENBasicServiceV1(unsigned long fixed_stationid,long fixed_stationtype,Ptr<Socket> socket_tx)
  {
  ...
  void
  DENBasicServiceV1::setStationProperties(unsigned long fixed_stationid,long fixed_stationtype)
  {
  ...
  void
  DENBasicServiceV1::setFixedPositionRSU(double latitude_deg, double longitude_deg)
  {
    m_btp->setFixedPositionRSU(latitude_deg,longitude_deg);
  }

  void
  DENBasicServiceV1::setStationID(unsigned long fixed_stationid)
  {
    m_station_id=fixed_stationid;
  ...
  void
  DENBasicServiceV1::setStationType(long fixed_stationtype)
  {
    m_stationtype=fixed_stationtype;
  ...
  void
  DENBasicServiceV1::setSocketTx (Ptr<Socket> socket_tx)
  {
    m_btp->setSocketTx(socket_tx);
  }

  void
  DENBasicServiceV1::setSocketRx (Ptr<Socket> socket_rx)
  {
    m_btp->setSocketRx(socket_rx);
    m_btp->addDENMRxCallback (std::bind(&DENBasicServiceV1::receiveDENM,this,std::placeholders::_1,std::placeholders::_2));
  }



  DENBasicServiceV1_error_t
  DENBasicServiceV1::appDENM_trigger(denData data, DEN_ActionID_t &actionid)
  {
    DENBasicServiceV1_error_t fillDENM_rval=DENMV1_NO_ERROR;
//    DENM_t *denm;

    if(!CheckMainAttributes ())
      {
        return DENMV1_ATTRIBUTES_UNSET;
      }
...
//      }
    ...
    if(!data.isDenDataRight())
        return DENMV1_WRONG_DE_DATA;

    auto denm = asn1cpp::makeSeq(DENMV1);
    if(bool(denm)==false)
      {
        return DENMV1_ALLOC_ERROR;
      }
    ...
    if (compute_timestampIts (m_real_time) > data.getDenmMgmtDetectionTime () + (data.getDenmMgmtValidityDuration ()*MILLI))
        return DENMV1_T_O_VALIDITY_EXPIRED;
    ...
    if(fillDENM_rval!=DENMV1_NO_ERROR)
      {
        //freeDENM(denm);
    ...
    {
      return DENMV1_ASN1_UPER_ENC_ERROR;
    }
    ...
    DENBasicServiceV1::setDENTimer(std::get<V_O_VALIDITY_INDEX>(m_originatingTimerTable[map_index]),Seconds(data.getDenmMgmtValidityDuration ()),&DENBasicServiceV1::T_O_ValidityStop,actionid);
    ...
    if(data.getDenmRepetitionDuration ()>0 && data.getDenmRepetitionInterval ()>0)
      {
        DENBasicServiceV1::setDENTimer(std::get<T_REPETITION_INDEX>(m_originatingTimerTable[map_index]),MilliSeconds(data.getDenmRepetitionInterval()),&DENBasicServiceV1::T_RepetitionStop,actionid);
        DENBasicServiceV1::setDENTimer(std::get<T_REPETITION_DURATION_INDEX>(m_originatingTimerTable[map_index]),MilliSeconds(data.getDenmRepetitionDuration ()),&DENBasicServiceV1::T_RepetitionDurationStop,actionid);
      }
  ...
  DENBasicServiceV1_error_t
  DENBasicServiceV1::appDENM_update(denData data, const DEN_ActionID_t actionid)
  {
    DENBasicServiceV1_error_t fillDENM_rval=DENMV1_NO_ERROR;
...
//      }
    ...
    if (entry_map_it == m_originatingITSSTable.end())
      {
        ...
        return DENMV1_UNKNOWN_ACTIONID;
      }
  ...
  DENBasicServiceV1_error_t
  DENBasicServiceV1::appDENM_termination(denData data, const DEN_ActionID_t actionid)
  {
...
//      }
    ...
    if (entry_originating_table != m_originatingITSSTable.end())
      ...
      {
        T_Repetition_Mutex.unlock();
        return DENMV1_NON_ACTIVE_ACTIONID_ORIGINATING;
      }
    ...
    if (entry_receiving_table != m_receivingITSSTable.end())
      ...
      {
        T_Repetition_Mutex.unlock();
        return DENMV1_NON_ACTIVE_ACTIONID_RECEIVING;
      }
    ...
    if(termination==1)
      {
        ...
        if(referenceTime==-1)
          {
            T_Repetition_Mutex.unlock();
            return DENMV1_WRONG_TABLE_DATA;
          }
      }
    else
      {
    ...
    DENBasicServiceV1::setDENTimer(std::get<V_O_VALIDITY_INDEX>(entry_timers_table->second),Seconds(data.getDenmMgmtValidityDuration ()),&DENBasicServiceV1::T_O_ValidityStop,actionid);

    /* 8. Calculate and start timers T_RepetitionDuration and T_Repetition when both parameters in denData are > 0 */
    if(data.getDenmRepetitionDuration ()>0 && data.getDenmRepetitionInterval ()>0)
      {
        DENBasicServiceV1::setDENTimer(std::get<T_REPETITION_INDEX>(entry_timers_table->second),MilliSeconds(data.getDenmRepetitionInterval()),&DENBasicServiceV1::T_RepetitionStop,actionid);
        DENBasicServiceV1::setDENTimer(std::get<T_REPETITION_DURATION_INDEX>(entry_timers_table->second),MilliSeconds(data.getDenmRepetitionDuration ()),&DENBasicServiceV1::T_RepetitionDurationStop,actionid);
      }
  ...
  void
  DENBasicServiceV1::receiveDENM(BTPDataIndication_t dataIndication,Address from)
  {
    Ptr<Packet> packet;
    asn1cpp::Seq<DENMV1> decoded_denm;
    denData den_data;
    ...
    if(!CheckMainAttributes ())
      {
        NS_LOG_ERROR("DENBasicServiceV1 has unset parameters. Cannot receive any data.");
        free(buffer);
        return;
      }
    ...
    DENBasicServiceV1::setDENTimer(m_T_R_Validity_Table[map_index],Seconds((long)validityDuration),&DENBasicServiceV1::T_R_ValidityStop,actionID);

    /* Fill den_data with the received information */
    bool location_ok,situation_ok,alacarte_ok;

    auto header = asn1cpp::getSeq(decoded_denm->header,ItsPduHeaderV1);
    DENBasicServiceV1::fillDenDataHeader (header, den_data);

    auto management = asn1cpp::getSeq(decoded_denm->denm.management,ManagementContainerV1);
    DENBasicServiceV1::fillDenDataManagement (management, den_data);


    auto location = asn1cpp::getSeqOpt(decoded_denm->denm.location,LocationContainerV1,&location_ok);
    if(location_ok)
        DENBasicServiceV1::fillDenDataLocation (location, den_data);

    auto situation = asn1cpp::getSeqOpt(decoded_denm->denm.situation,SituationContainerV1,&situation_ok);
    if(situation_ok)
        DENBasicServiceV1::fillDenDataSituation (situation, den_data);

    auto alacarte = asn1cpp::getSeqOpt(decoded_denm->denm.alacarte,AlacarteContainerV1,&alacarte_ok);
    if(alacarte_ok)
        DENBasicServiceV1::fillDenDataAlacarte (alacarte, den_data);
  ...
  void
  DENBasicServiceV1::T_O_ValidityStop(DEN_ActionID_t entry_actionid)
  {
    T_Repetition_Mutex.lock ();
  ...
  void
  DENBasicServiceV1::T_RepetitionDurationStop(DEN_ActionID_t entry_actionid)
  {
    std::pair <unsigned long, long> map_index = std::make_pair((unsigned long)entry_actionid.originatingStationID,(long)entry_actionid.sequenceNumber);
  ...
  void
  DENBasicServiceV1::T_RepetitionStop(DEN_ActionID_t entry_actionid)
  {
    T_Repetition_Mutex.lock();
    std::pair <unsigned long, long> map_index = std::make_pair((unsigned long)entry_actionid.originatingStationID,(long)entry_actionid.sequenceNumber);
    T_Repetition_Mutex.unlock ();
  ...
  void
  DENBasicServiceV1::T_R_ValidityStop(DEN_ActionID_t entry_actionid)
  {
    T_Repetition_Mutex.lock();
    std::pair <unsigned long, long> map_index = std::make_pair((unsigned long)entry_actionid.originatingStationID,(long)entry_actionid.sequenceNumber);
  ...
  void
  DENBasicServiceV1::cleanup(void)
  {
  ...
  void
  DENBasicServiceV1::fillDenDataHeader(asn1cpp::Seq<ItsPduHeaderV1>denm_header, denData &denm_data)
  {
  ...
  void
  DENBasicServiceV1::fillDenDataManagement(asn1cpp::Seq<ManagementContainerV1> denm_mgmt_container, denData &denm_data)
  {
  ...
  void
  DENBasicServiceV1::fillDenDataSituation(asn1cpp::Seq<SituationContainerV1> denm_situation_container, denData &denm_data)
  {
    ...
    auto eventHist = asn1cpp::getSeqOpt(denm_situation_container->eventHistory,EventHistoryV1,&ok);
    if(ok)
      {
  ...
  void
  DENBasicServiceV1::fillDenDataLocation(asn1cpp::Seq<LocationContainerV1> denm_location_container, denData &denm_data)
  {
    denData::denDataLocation location;
    bool ok;

    auto traces = asn1cpp::getSeq(denm_location_container->traces,TracesV1);
    for(int i=0; i<3;i++)
      {
        auto pathHistory = asn1cpp::sequenceof::getSeq(*traces,PathHistoryV1,i);
        std::vector<DEN_PathPoint_t> pathHistory_data;
        for(int j=0;j<3;j++)
          {
            auto pathPoint = asn1cpp::sequenceof::getSeq(*pathHistory,PathPointV1,j);
  ...
  void
  DENBasicServiceV1::fillDenDataAlacarte(asn1cpp::Seq<AlacarteContainerV1> denm_alacarte_container, denData &denm_data)
  {
    ...
    auto impactReduction = asn1cpp::getSeqOpt(denm_alacarte_container->impactReduction,ImpactReductionContainerV1,&ok);
    if(ok)
      {
...
//        byte2 = asn1cpp::bitstring::getterByteMask(impactReduction->positionOfOccupants,2);
    ...
    auto roadworks = asn1cpp::getSeqOpt(denm_alacarte_container->roadWorks,RoadWorksContainerExtendedV1,&ok);
    if(ok)
      {
        ...
        auto incidentIndication = asn1cpp::getSeqOpt(roadworks->incidentIndication,CauseCodeV1,&roadworks_ok);
        if(roadworks_ok)
          {
        ...
        auto recommendedPath = asn1cpp::getSeqOpt(roadworks->recommendedPath,ItineraryPathV1,&roadworks_ok);
        if(roadworks_ok)
          {
            ...
            for(int i=0;i<recommended_size;i++)
              {
                ...
                auto refPos = asn1cpp::sequenceof::getSeq(roadworks->recommendedPath,ReferencePositionV1,i);
        ...
        auto startingSpeedLimit = asn1cpp::getSeqOpt(roadworks->startingPointSpeedLimit,DeltaReferencePositionV1,&roadworks_ok);
        if(roadworks_ok)
          {
        ...
        auto refDenms = asn1cpp::getSeqOpt(roadworks->referenceDenms,ReferenceDenmsV1,&roadworks_ok);
        if(roadworks_ok)
          {
            ...
            for(int i=0;i<refDenms_size;i++)
              {
                ...
                auto actionId = asn1cpp::sequenceof::getSeq(roadworks->referenceDenms,ActionIDV1,i);
    ...
    auto stationaryVehicle = asn1cpp::getSeqOpt(denm_alacarte_container->stationaryVehicle,StationaryVehicleContainerV1,&ok);
    if(ok)
      {
        bool stationary_ok;
        ...
        auto stationaryCause = asn1cpp::getSeqOpt(stationaryVehicle->stationaryCause,CauseCodeV1,&stationary_ok);
        if(stationary_ok)
          {
        ...
        auto dangerousGoods = asn1cpp::getSeqOpt(stationaryVehicle->carryingDangerousGoods,DangerousGoodsExtendedV1,&stationary_ok);
        if(stationary_ok)
          {
            bool dangerous_ok;
        ...
        auto vehicleId = asn1cpp::getSeqOpt(stationaryVehicle->vehicleIdentification,VehicleIdentificationV1,&stationary_ok);
        if(stationary_ok)
          {
            bool vehicleId_ok;
